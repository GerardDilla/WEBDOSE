...
class Registrar_Model extends CI_Model
{
  ...
  public function get_departments_choice()
  {

    $this->db->select('UPPER(User_Department) as dept');
    $this->db->from('Users');
    $this->db->where('tabValid', 1);
    $this->db->where('User_Department <>', '');
    $this->db->group_by('TRIM(UPPER(User_Department))', '1');
    $this->db->order_by('TRIM(User_Department)', 'ASC');
    $query = $this->db->get();
    return $query->result_array();
  }

  public function get_bypass_users($Filter)
  {
    $this->db->select('
      U.User_ID,
      U.UserName,
    ...
    $this->db->where('U.tabValid', 1);
    if ($Filter['Department'] != '') {
      $this->db->like('U.User_Department', $Filter['Department']);
    }
    if ($Filter['Searchkey'] != '') {
      $this->db->group_start();
      $this->db->like('U.UserName', $Filter['Searchkey']);
      $this->db->or_like('U.User_FullName', $Filter['Searchkey']);
      $this->db->group_end();
    }
  ...
  {
    $this->db->join('module_bypass as MB', 'MB.School_ID = SI.School_ID AND MB.User_ID = ' . $input['UserID'] . ' AND MB.valid = 1', 'LEFT');
    $this->db->order_by('SI.School_ID', 'ASC');
    $query = $this->db->get('School_Info as SI');
    return $query->result_array();
  }
  public function get_school_id($schoolcode)
  {
  ...
  public function get_user_permission($UserID, $School_ID)
  {
    $this->db->where('MB.User_ID', $UserID);
    $this->db->where('MB.School_ID', $School_ID);
    $this->db->where('MB.valid', 1);
    $query = $this->db->get('module_bypass as MB');
    return $query->result_array();
  }
  public function update_permission($userid)
  {
    $this->db->trans_start();
  ...
  {
    $this->db->insert('module_bypass', $insert);
